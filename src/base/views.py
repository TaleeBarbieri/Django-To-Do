...
class RegisterPage(FormView):
    ...
    def get(self, *args, **kwargs):
        ...
        return super(RegisterPage, self).get(*args, **kwargs)       # This checks if user is authenticated is so he will not be aloud to access other pages
...
class TaskList(LoginRequiredMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if search_input:        # This checks if the user entered something to search for
            context['tasks'] = context['tasks'].filter(     # Checks if user entered the title of one of the tasks that is in the variable 'search-input'
